...
数据可用性会决定报告允许输出的判断深度：只有单周基础数据时，只输出首诊假设；连续4周后可做店铺自身历史对比；连续12周后可做趋势判断；有平台同层级/行业数据才做行业判断；有毛利率和投放归因才做投放效率判断；有商品成本和库存才做利润与库存策略建议。

固定原则：没有数据源，就不输出强判断；有数据源，就标注来源和口径。
