...
本周曝光和访客都被活动拉起来了，这是好信号，访客规模也已具备初步分析价值。但在没有历史均值和同层级类目均值前，不能直接说“流量达标”。如果新增访客主要来自价格敏感人群或活动低意向人群，继续放大活动可能只会增加浏览，不一定增加成交。
...
ROI口径不统一时，直接加预算或停预算都可能误判。这里应先确认平台归因口径、广告直接成交、间接成交、毛利率和渠道拆分，再决定哪些计划可以放量、哪些计划需要缩减；在口径未核清前，不给固定比例砍预算建议。

### 售后/复购｜退款率健康，但不能直接等同复购健康

从单周首诊看，退款率暂未暴露明显售后风险，这说明用户买完后没有明显不满。但在没有行业均值和店铺历史均值前，不直接判断“行业合理”或“复购健康”；后续还需要看老客成交占比、复购率和二次购买周期。
...
本报告由AI经营诊断流程生成初稿，并经经营逻辑校验后输出。本报告属于单周首诊判断，不替代完整月度复盘或平台后台深度审计，不承诺一定提升GMV。未接入历史均值、行业均值、平台同层级数据、毛利率或投放归因明细前，报告不直接输出“达标、行业合理、明显偏低、加预算、砍预算”等强判断。
"""
