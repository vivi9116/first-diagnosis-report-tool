...
本报告由AI经营诊断流程生成初稿，并经经营逻辑校验后输出。本报告当前按“单周基础数据首诊”处理，只输出首诊假设、当前样本观察和低风险验证动作，不替代完整月度复盘或平台后台深度审计，不承诺一定提升GMV。未形成连续4周店铺自身基准前，不输出店铺历史对比；未形成连续12周数据前，不输出趋势判断和波动区间；未接入行业/平台同层级数据前，不输出行业或同层级强判断；未同时接入毛利率和投放归因前，不输出投放效率强判断；未同时接入商品成本和库存数据前，不输出利润和库存策略建议。没有数据源，就不输出强判断；有数据源，才标注来源和口径后输出对应深度判断。
"""
